...
public class Z3DifferentialTests
{
    ...
    public void Euf_Agrees(string text, bool _)
    {
        ...
        Assert.Equal(EufSolver.IsSatisfiable(f), Z3Smt.IsSatisfiable(f, Z3SmtTheory.Euf));
        Assert.Equal(EufSolver.IsValid(f), Z3Smt.IsValid(f, Z3SmtTheory.Euf));
    }
    ...
    public void Lra_Agrees(string text)
    {
        ...
        Assert.Equal(LraSolver.IsSatisfiable(f), Z3Smt.IsSatisfiable(f, Z3SmtTheory.Lra));
    }
    ...
    public void Combined_Agrees()
    {
        ...
        Assert.Equal(CombinedSolver.IsSatisfiable(mixed), Z3Smt.IsSatisfiable(mixed, Z3SmtTheory.Combined));
        Assert.False(Z3Smt.IsSatisfiable(mixed, Z3SmtTheory.Combined));   // x = y ⇒ f(x) = f(y), contradicting f(x) ≠ f(y)
    }
    ...
    public void Arrays_Agrees(string text, bool expectedValid)
    {
        ...
        Assert.Equal(ArraySolver.IsSatisfiable(f), Z3Smt.IsSatisfiable(f, Z3SmtTheory.Arrays));
        Assert.Equal(expectedValid, Z3Smt.IsValid(f, Z3SmtTheory.Arrays));
    }
}
